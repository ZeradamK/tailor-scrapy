...
while True:
    ...
    if headers:
        # Ensure a reasonable default desktop/mobile UA if none provided
    ...
    try:
        ...
        # Mobile emulation knob via env (helps on JS-heavy PDPs)
        if headers.get("X-Viewport", "").lower() == "mobile":
            env["SCRAPY_MOBILE"] = "true"
        if proxy_url:
